...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    cur_one = player_score % 10
    oppo_tens = (opponent_score // 10) % 10
    score = 3*abs(cur_one - oppo_tens)
    if score == 0:
        return 1
    else:
        return score
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    for num_rolls in range(1, 11):
        ave_roll_dice = make_averaged(roll_dice, times_called)
        ave = ave_roll_dice(num_rolls,dice)
        if ave > max_avg:
            max_avg = ave
            max_mun = num_rolls
    return max_mun
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    tmp_score = sus_points(score + boar_brawl(score, opponent_score))
    if tmp_score - score >= threshold:
        return 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
